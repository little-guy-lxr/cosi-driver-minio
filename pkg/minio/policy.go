...
func (s *C) SettBucketPolicy(bucket string, policy *BucketPolicy) error {
	serializedPolicy, _ := json.Marshal(*policy)
	consumablePolicy := string(serializedPolicy)
	err := s.client.SetBucketPolicy(context.TODO(), bucket, consumablePolicy)
	if err != nil {
		return err
	}
	return nil
...
func (s *C) GetBucketPolicy(bucket string) (*BucketPolicy, error) {
	out, err := s.client.GetBucketPolicy(context.TODO(), bucket)
	if err != nil {
		return nil, err
	}
	policy := &BucketPolicy{}
	err = json.Unmarshal([]byte(out), policy)
	if err != nil {
		return nil, err
	}
	return policy, nil
